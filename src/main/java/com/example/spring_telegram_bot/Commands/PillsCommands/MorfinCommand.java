...
@Component
public class MorfinCommand implements WorKerCommand {
    @Override
    public SendMessage start(Update updata) {
        if (!updata.getMessage().getText().equals("Морфин")){
            return null;
        }
        UserModel userModel = UserHelper.findUser(updata.getMessage().getFrom().getId().toString());
        userModel.setPillsEnum(PillsEnum.MORFIN);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> list = PillsHelper.getFreetimes(PillsEnum.MORFIN);
