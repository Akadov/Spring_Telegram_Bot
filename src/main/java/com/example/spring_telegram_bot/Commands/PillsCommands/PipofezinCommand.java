...
@Component
public class PipofezinCommand implements WorKerCommand {
    @Override
    public SendMessage start(Update updata) {
        if (!updata.getMessage().getText().equals("Пипофезин")){
            return null;
        }
        UserModel userModel = UserHelper.findUser(updata.getMessage().getFrom().getId().toString());
        userModel.setPillsEnum(PillsEnum.PIPOFEZIN);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> list = PillsHelper.getFreetimes(PillsEnum.PIPOFEZIN);
