...
@Component
public class HlorpromazinCommand implements WorKerCommand {
    @Override
    public SendMessage start(Update updata) {
        if (!updata.getMessage().getText().equals("Хлорпромазин")){
            return null;
        }
        UserModel userModel = UserHelper.findUser(updata.getMessage().getFrom().getId().toString());
        userModel.setPillsEnum(PillsEnum.HLORPROMAZIN);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> list = PillsHelper.getFreetimes(PillsEnum.HLORPROMAZIN);
