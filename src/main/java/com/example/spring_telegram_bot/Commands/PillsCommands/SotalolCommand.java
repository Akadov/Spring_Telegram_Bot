...
@Component
public class SotalolCommand implements WorKerCommand {
    @Override
    public SendMessage start(Update updata) {
        if (!updata.getMessage().getText().equals("Соталол")){
            return null;
        }
        UserModel userModel = UserHelper.findUser(updata.getMessage().getFrom().getId().toString());
        userModel.setPillsEnum(PillsEnum.SOTALOL);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> list = PillsHelper.getFreetimes(PillsEnum.SOTALOL);
