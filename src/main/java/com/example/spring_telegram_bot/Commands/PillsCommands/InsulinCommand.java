...
@Component
public class InsulinCommand implements WorKerCommand {
    @Override
    public SendMessage start(Update updata) {
        if (!updata.getMessage().getText().equals("Инсулин")){
            return null;
        }
        UserModel userModel = UserHelper.findUser(updata.getMessage().getFrom().getId().toString());
        userModel.setPillsEnum(PillsEnum.INSULIN);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> list = PillsHelper.getFreetimes(PillsEnum.INSULIN);
